...
# Mensajes:
not_enough_philosophers = '¡Necesitamos más filósofos!'
    ...
    def run(self):
        while True:
            with self.forks['left']:
                ...
                print(Philosopher.fork_acquired.format(self, fork_acquired_left))
                with self.forks['right']:
                    fork_acquired_right = table.forks.index(self.forks['right'])
                    print(Philosopher.fork_acquired.format(self, fork_acquired_right))
                    self.eat()
                    print(Philosopher.fork_released.format(self, fork_acquired_right))
                print(Philosopher.fork_released.format(self, fork_acquired_left))
...
class Table:
    ...
    def setup_philosophers(self):
        # Entregamos los tenedores a los filosofos.
        i = int()
    ...
    def start_discussion(self):
        # ¡Nuestro filosofos comienzan a discutir mientras cenan!
        for philosopher in self.philosophers.values():
            philosopher.start()


if __name__ == '__main__':
    # Creamos a los filosofos y la mesa en la que cenaran.
    names = ('Benjamín', 'Freddie', 'Antonio', 'Andres', 'Ariel')
